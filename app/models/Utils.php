...
class Utils extends Model
{
    ...
    static function rpc($method = '', $data = [])
    {
        $host = DI::getDefault()->get('config')->api->rpc_host;
        $protocol = DI::getDefault()->get('config')->api->rpc_protocol;

        list($class, $action) = explode('/', trim($method, '/'));
        switch ($protocol) {
            case 'yar':
